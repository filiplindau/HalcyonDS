...
# ------------------------------------------------------------------
    def init_device(self):
        ...
        try:
            ...
            self.halcyonTracking.maxNumberAdjusts = 30
            self.halcyonTracking.moveSteps = self.picoMoveSteps
            self.halcyonTracking.updateDeadtime = self.picoMoveDelay
        ...
        while self.stopStateThreadFlag is False:
            ...
            try:
                ...
                with self.attrLock:
                    ...
                    self.redpitayaDevice.write_attribute('samplerate', self.sampleRate)
...
#                    self.picomotorDevice.write_attribute('CurrentMotor', 0, wait=False)
    ...
    def updateRedpitaya(self, arg):
        ...
        with self.attrLock:
            if attr.quality == PyTango.AttrQuality.ATTR_VALID:
                ...
                fref = self.refFrequency
                Vpp = self.jitterPeakVoltage
                self.debug_stream("Jitter peak voltage: {0}".format(self.jitterPeakVoltage))
                # self.jitter = 1 / (2 * np.pi * fref) * self.errorTrace.std() / (Vpp / 2)
                self.jitter = self.calcPhaseNoise(attr.value)
    ...
    def calcPhaseNoise(self, w):
        n = w.shape[0]
        fs = self.sampleRate
        dt = 1.0 / fs
        t = np.arange(n) * dt
        A = self.jitterPeakVoltage
        f0 = self.refFrequency

        fw, Pw = periodogram(w / (A / 2), fs)
        f_arg = np.argmax(np.abs(fw - self.jitterLowFrequency))
        j = np.sqrt(np.cumsum(Pw[::-1])) / (2 * np.pi * f0)
        j = j[::-1]
        try:
            j0 = j[f_arg]
...
class HalcyonDSClass(PyTango.DeviceClass):
    ...
    device_property_list = {
        ...
        'picoMoveSteps':
            [PyTango.DevShort,
             "Number of steps moved when autoadjusting pico motor. Fewer steps is more stable but takes longer time. 1-2 is normal.",
             [2]],
        'picoMoveDelay':
            [PyTango.DevDouble,
             "Time delay between moves of the picomotor during autoadjustment. 1.0 s is normal.",
             [1.0]],
        ...
        'jitterPeakVoltage':
            [PyTango.DevDouble,
             "Peak-peak voltage maximum for the error monitor signal",
             [0.8]],
        'sampleRate':
            [PyTango.DevDouble,
             "Samplerate for the redpitaya where the jitter is read from",
             [15258.0]],
        'refFrequency':
            [PyTango.DevDouble,
             "Reference frequency of the accelerator in Hz",
             [2.9985e9]],
        'jitterLowFrequency':
            [PyTango.DevDouble,
             "Lower bound of jitter calculation, Hz",
             [10.0]],
    }
